...
noaa_forecast_path <- file.path(noaa_data_location, lubridate::as_date(forecast_start_datetime_UTC),forecast_hour)


# Remove the ens00.nc file so there there are 30 forecast ensembles up to 35 days into the future
...
if(length(forecast_files) > 0){
  ...
  observed_met_file <- file.path(config$qaqc_data_location,"observed-met_prla.nc")
  ...
  obs <- create_obs_matrix_test(cleaned_observations_file_long,
                                  obs_config,
